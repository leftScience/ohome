...
APP_UPDATE_USE_PACKAGE_INSTALLER="${APP_UPDATE_USE_PACKAGE_INSTALLER:-}"
DRY_RUN=""
...
if [[ -n "$APP_UPDATE_MANIFEST_URL" ]]; then
  BUILD_ARGS+=(--dart-define=APP_UPDATE_MANIFEST_URL="$APP_UPDATE_MANIFEST_URL")
fi

if [[ -n "$APP_UPDATE_USE_PACKAGE_INSTALLER" ]]; then
  BUILD_ARGS+=(--dart-define=APP_UPDATE_USE_PACKAGE_INSTALLER="$APP_UPDATE_USE_PACKAGE_INSTALLER")
fi
